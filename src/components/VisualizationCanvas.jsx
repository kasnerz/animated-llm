...
function VisualizationCanvas() {
  ...
  useEffect(() => {
    ...
    // 2. Render embeddings
    const outerMeta = renderOuterEmbeddings(
      embeddingGroup,
      step,
      layout,
      width,
    ...
    if (isInitialStep) {
      ...
      gsap.set(svgRef.current.querySelectorAll('.token-id-arrow'), {
        opacity: subStep >= 1 ? 1 : 0,
      });
      gsap.set(svgRef.current.querySelectorAll('.embedding-group, .embedding-group *'), {
        opacity: subStep >= 2 ? 1 : 0,
        y: subStep >= 2 ? 0 : -8,
      });
      gsap.set(svgRef.current.querySelectorAll('.id-to-emb-arrow'), {
        opacity: subStep >= 2 ? 1 : 0,
      });

      gsap.set(svgRef.current.querySelectorAll('.transformer-group .transformer-box'), {
      ...
      gsap.set(
        ...
        { opacity: subStep >= 3 ? 1 : 0, y: subStep >= 3 ? 0 : -8 }
      );
      gsap.set(svgRef.current.querySelectorAll('.outer-to-block-arrow'), {
        opacity: subStep >= 3 ? 1 : 0,
      });
      gsap.set(
        svgRef.current.querySelectorAll(
          '.transformer-group .attention-mash, .transformer-group .attention-mash *'
        ),
        { opacity: subStep >= 3 ? 1 : 0 }
      );
      ...
      gsap.set(
        ...
        { opacity: subStep >= 4 ? 1 : 0, y: subStep >= 4 ? 0 : 8 }
      );
      gsap.set(svgRef.current.querySelectorAll('.ffn-arrow'), { opacity: subStep >= 4 ? 1 : 0 });

      gsap.set(
        svgRef.current.querySelectorAll('.bottom-embedding-group, .bottom-embedding-group *'),
        { opacity: subStep >= 5 ? 1 : 0, y: subStep >= 5 ? 0 : 8 }
      );
      ...
      gsap.set(svgRef.current.querySelectorAll('.extracted-path-arrow'), {
        opacity: subStep >= 7 ? 1 : 0,
      });
      gsap.set(svgRef.current.querySelectorAll('.extracted-horizontal'), {
        opacity: subStep >= 7 ? 1 : 0,
      });
      gsap.set(svgRef.current.querySelectorAll('.logprob-arrow'), {
        opacity: subStep >= 7 ? 1 : 0,
      });
      gsap.set(svgRef.current.querySelectorAll('.logprob-vector'), {
        opacity: subStep >= 7 ? 1 : 0,
      });
      gsap.set(svgRef.current.querySelectorAll('.distribution-bar'), {
      ...
      gsap.set(
        svgRef.current.querySelectorAll(
          '.distribution-token-label, .distribution-percentage-label'
        ),
        { opacity: subStep >= 8 ? 1 : 0 }
      );
    } else {
      ...
      gsap.set(svgRef.current.querySelectorAll('.token.new-token'), {
        opacity: subStep >= 0 ? 1 : 0,
      });
      gsap.set(svgRef.current.querySelectorAll('.token-id.prev-token'), { opacity: 1 });
      gsap.set(svgRef.current.querySelectorAll('.token-id.new-token'), {
        opacity: subStep >= 1 ? 1 : 0,
      });
      gsap.set(svgRef.current.querySelectorAll('.token-id-arrow.prev-token'), { opacity: 1 });
      gsap.set(svgRef.current.querySelectorAll('.token-id-arrow.new-token'), {
        opacity: subStep >= 1 ? 1 : 0,
      });

      gsap.set(svgRef.current.querySelectorAll('.embedding-group .embedding-col.prev-token'), {
        opacity: 1,
        y: 0,
      });
      gsap.set(svgRef.current.querySelectorAll('.embedding-group .embedding-col.new-token'), {
        opacity: subStep >= 2 ? 1 : 0,
        y: subStep >= 2 ? 0 : -8,
      });
      gsap.set(svgRef.current.querySelectorAll('.id-to-emb-arrow.prev-token'), { opacity: 1 });
      gsap.set(svgRef.current.querySelectorAll('.id-to-emb-arrow.new-token'), {
        opacity: subStep >= 2 ? 1 : 0,
      });
      ...
      gsap.set(
        svgRef.current.querySelectorAll(
          '.transformer-group .inside-top-embeddings .embedding-col.prev-token'
        ),
        { opacity: 1, y: 0 }
      );
      gsap.set(
        svgRef.current.querySelectorAll(
          '.transformer-group .inside-top-embeddings .embedding-col.new-token'
        ),
        { opacity: subStep >= 3 ? 1 : 0, y: subStep >= 3 ? 0 : -8 }
      );
      gsap.set(svgRef.current.querySelectorAll('.outer-to-block-arrow.prev-token'), { opacity: 1 });
      gsap.set(svgRef.current.querySelectorAll('.outer-to-block-arrow.new-token'), {
        opacity: subStep >= 3 ? 1 : 0,
      });
      gsap.set(
        svgRef.current.querySelectorAll(
      ...
      gsap.set(svgRef.current.querySelectorAll('.ffn-arrow.new-token'), {
        opacity: subStep >= 4 ? 1 : 0,
      });

      gsap.set(
        svgRef.current.querySelectorAll('.bottom-embedding-group .embedding-col.prev-token'),
        { opacity: 1, y: 0 }
      );
      gsap.set(
        svgRef.current.querySelectorAll('.bottom-embedding-group .embedding-col.new-token'),
        { opacity: subStep >= 5 ? 1 : 0, y: subStep >= 5 ? 0 : 8 }
      );
    ...
    if (isInitialStep) {
      ...
      if (subStep === 0) {
        tl.to(svgRef.current.querySelectorAll('.token'), {
          opacity: 1,
          duration: animDuration,
          stagger: 0.02,
        });
      } else if (subStep === 1) {
        tl.to(svgRef.current.querySelectorAll('.token-id-arrow'), {
          opacity: 1,
          duration: animDuration * 0.5,
        });
        tl.to(
          svgRef.current.querySelectorAll('.token-id'),
          { opacity: 1, duration: animDuration, stagger: 0.02 },
          '<'
        );
      } else if (subStep === 2) {
        tl.to(svgRef.current.querySelectorAll('.embedding-group, .embedding-group *'), {
          opacity: 1,
          y: 0,
          duration: animDuration,
        });
        tl.to(
          svgRef.current.querySelectorAll('.id-to-emb-arrow'),
          { opacity: 1, duration: animDuration * 0.5 },
          '<'
        );
      } else if (subStep === 3) {
        tl.to(svgRef.current.querySelectorAll('.transformer-group .transformer-box'), {
          opacity: 1,
          scaleY: 1,
        ...
        tl.to(
          svgRef.current.querySelectorAll('.outer-to-block-arrow'),
          { opacity: 1, duration: animDuration * 0.5 },
          '<'
        );
        tl.to(
        ...
        tl.to(
          svgRef.current.querySelectorAll('.ffn-arrow'),
          { opacity: 1, duration: animDuration * 0.5 },
          '<'
        );
      } else if (subStep === 5) {
      ...
      if (subStep === 0) {
        tl.to(svgRef.current.querySelectorAll('.token.new-token'), {
          opacity: 1,
          duration: animDuration,
        });
      } else if (subStep === 1) {
        tl.to(svgRef.current.querySelectorAll('.token-id-arrow.new-token'), {
          opacity: 1,
          duration: animDuration * 0.5,
        });
        tl.to(
          svgRef.current.querySelectorAll('.token-id.new-token'),
          { opacity: 1, duration: animDuration },
          '<'
        );
      } else if (subStep === 2) {
        tl.to(svgRef.current.querySelectorAll('.embedding-group .embedding-col.new-token'), {
          opacity: 1,
          y: 0,
          duration: animDuration,
        });
        tl.to(
          svgRef.current.querySelectorAll('.id-to-emb-arrow.new-token'),
          { opacity: 1, duration: animDuration * 0.5 },
          '<'
        );
      } else if (subStep === 3) {
        ...
        tl.to(
          svgRef.current.querySelectorAll('.outer-to-block-arrow.new-token'),
          { opacity: 1, duration: animDuration * 0.5 },
          '<'
        );
      } else if (subStep === 4) {
        ...
        tl.to(
          svgRef.current.querySelectorAll('.ffn-arrow.new-token'),
          { opacity: 1, duration: animDuration * 0.5 },
          '<'
        );
      } else if (subStep === 5) {
        tl.to(svgRef.current.querySelectorAll('.bottom-embedding-group .embedding-col.new-token'), {
          opacity: 1,
          y: 0,
          duration: animDuration,
        });
    ...
    if (subStep === 6 && extractedNode) {
      ...
      tl.to(
        svgRef.current.querySelectorAll('.extracted-path-arrow'),
        { opacity: 1, duration: animDuration * 0.4 },
        '>-0.1'
      );
      tl.to(
        svgRef.current.querySelectorAll('.extracted-horizontal'),
        { opacity: 1, scale: 1.35, transformOrigin: '50% 50%', duration: animDuration * 0.7 },
        '<'
      );
    ...
    if (subStep === 7) {
      tl.to(svgRef.current.querySelectorAll('.logprob-arrow'), {
        opacity: 1,
        duration: animDuration * 0.5,
      });
      tl.to(
        svgRef.current.querySelectorAll('.logprob-vector'),
        { opacity: 1, duration: animDuration },
        '<'
      );
    }
    ...
    if (subStep === 8) {
      tl.fromTo(
        svgRef.current.querySelectorAll('.distribution-bar'),
      ...
      tl.to(
        ...
        { opacity: 1, duration: animDuration * 0.8, stagger: 0.015 },
        '<+0.2'
      );
    }
    ...
    gsapRef.current = tl;
    // Render functions are defined inline within this effect, so no need to add them to deps
  ...
  const renderOuterEmbeddings = (group, step, layout, width, shouldCollapse, maxVisibleTokens) => {
    ...
    visibleIndices.forEach((actualIndex, i) => {
      ...
      group
        ...
        .style('stroke-width', 1.5)
        .style('opacity', 0.7);
      drawArrow(group, x, arrowStart, x, meta.topY, {
        className: `id-to-emb-arrow ${isNew ? 'new-token' : 'prev-token'}`,
      });
    });
  ...
  const drawArrow = (group, x1, y1, x2, y2, opts = {}) => {
    ...
    arrowG.append('polygon').attr('points', points).style('fill', '#ccc').style('opacity', 0.7);
  ...
  const renderBottomEmbeddings = (group, step, layout, width, blockMeta) => {
    ...
    visibleIndices.forEach((actualIndex, i) => {
      ...
      const meta = drawEmbeddingColumn(group, x, topY, vals, {
        expanded: false,
        tokenColor,
        className: isNew ? 'new-token' : 'prev-token',
      });
  ...
  const renderOutput = (group, step, layout, width, bottomInfo, subStep) => {
    ...
    for (let i = visibleIndices.length - 1; i >= 0; i--) {
      if (visibleIndices[i] >= 0) {
        rightmostActualIndex = visibleIndices[i];
        break;
      }
    }
    ...
    if (subStep >= 6) {
      ...
      const sampleValues = (
        rightmostActualIndex >= 0 ? step.embeddings?.[rightmostActualIndex]?.values || [] : []
      ).slice(0, 8);
      hv1 = drawHorizontalVector(group, horizCenterX, horizY, sampleValues, {
  ...
  const renderExpandButton = (group, layout, width, isCurrentlyExpanded) => {
    ...
    if (isCurrentlyExpanded) {
      ...
      buttonG
        ...
        .attr(
          'd',
          'M 14 15 L 18 15 M 22 15 L 26 15 M 18 12 L 15 15 L 18 18 M 22 12 L 25 15 L 22 18'
        )
      ...
      buttonG
        ...
        .attr(
          'd',
          'M 18 15 L 14 15 M 26 15 L 22 15 M 14 12 L 11 15 L 14 18 M 26 12 L 29 15 L 26 18'
        )
        .style('stroke', 'var(--text-secondary)')
        .style('stroke-width', 1.5)
        .style('fill', 'none')
