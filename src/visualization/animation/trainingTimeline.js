...
export function setInitialStates(svgElement, subStep, isInitialStep) {
  ...
  setIfAny(svgElement, `${SEL.positionalEmb} circle`, { opacity: prev >= 3 ? 0.9 : 0 });
  setIfAny(svgElement, `${SEL.positionalEmb} text`, { opacity: prev >= 3 ? 1 : 0 });
  ...
  setIfAny(svgElement, SEL.transformerShadowBox, { opacity: prev >= 6 ? 1 : 0 });
  setIfAny(svgElement, SEL.transformerStackLabel, { opacity: prev >= 6 ? 1 : 0 });

  // Extraction to logprobs
  setIfAny(svgElement, SEL.extractedEmbedding, { opacity: prev >= 7 ? 1 : 0 });
  setIfAny(svgElement, SEL.extractedPathArrow, { opacity: prev >= 7 ? 1 : 0 });
  setIfAny(svgElement, SEL.extractedHorizontal, { opacity: prev >= 7 ? 1 : 0 });
  ...
  setIfAny(svgElement, SEL.distributionLabels, { opacity: prev >= 9 ? 1 : 0 });
  setIfAny(svgElement, SEL.distributionItem, { opacity: prev >= 9 ? 1 : 0 });
...
/**
 * Build a compact GSAP timeline that plays through sub-steps 0..9 automatically.
 * Skips output highlight (10) and append preview (11..12) to reflect teacher forcing.
 */
