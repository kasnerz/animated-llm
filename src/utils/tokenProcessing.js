 ...
 *
 * This is the FALLBACK path. Data generated by the current servers carries an
 * explicit `special_idx` list derived from the tokenizer's own inventory, which
 * is authoritative. These patterns only classify data that predates that field.
 *
 * Special tokens include:
 * - Angle-bracket marker tokens like <BOS_TOKEN>, <EOS_TOKEN>, <bos>, <eos>
 * - Chat markers like <|im_start|>, <|im_end|>, <|start_header_id|>, <|end_header_id|>, <|eot_id|>
 * - Gemma-style asymmetric markers like <|turn>, <turn|>, <|channel>, <channel|>
...
export function isSpecialToken(token) {
  ...
  // <bos>, <eos>, <pad>, <mask> (Gemma-style lowercase markers)
  if (/^<(bos|eos|pad|unk|mask)>$/.test(token)) return true;
  ...
  // Gemma 4 uses asymmetric delimiters: <|turn>...<turn|>, <|channel>...<channel|>
  if (/^<\|[a-z_]+>$/.test(token)) return true;
  if (/^<[a-z_]+\|>$/.test(token)) return true;
 ...
 * Recognizes <|im_start|> (ChatML), <|start_header_id|> (Llama 3) and <|turn> (Gemma 4).
 */
export function isChatStartMarker(token) {
  return token === '<|im_start|>' || token === '<|start_header_id|>' || token === '<|turn>';
}
 ...
 */
const CHAT_ROLES = new Set(['user', 'assistant', 'system', 'developer', 'tool', 'model']);
